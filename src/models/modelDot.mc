...
let resistorToDot = lam quote. lam name. lam value. lam custom_settings.
    let settings = match custom_settings with Some setting then setting else
                "style=filled color=black fillcolor=none shape=rect height=0.1 width=0.3 " in
...
let componentToDot = lam comp. lam quote. lam fig_settings.
    match comp with Component (comp_type,name,maybe_value) then
        let figure_setting = 
            ...
            match fig with Some (_,setting) then Some setting else None() in
...
let circGetDot = lam circ. lam id. lam fig_settings.
    ...
    let dotComponents = concatList (map (lam c. componentToDot c quote fig_settings) components) in
